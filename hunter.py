...
def send_new_menu(bounties_list, comparison_text, show_deep=True):
    global CURRENT_MENU_ID, CURRENT_MENU_TEXT
...
def run_bounty_hunter():
    ...
    while True:
        ...
        for b in bounties:
            ...
            if deep_scan or (v["score"] >= 7 and v["is_winnable"]):
                b.update(v)
                display_bounties.append(b)
                current_ids.append(b["id"])

        # NEW LOGIC: Is this a duplicate?
        if display_bounties:
            is_dup = set(current_ids) == set(PREVIOUS_BOUNTY_IDS)
            
            if is_dup and not (force_scan or deep_scan):
                # Just update the timestamp on the existing message, no new notification
                ...
                send_new_menu(display_bounties, "Deep Scan Results" if deep_scan else "New Bounties Found")
        ...
        if idle_res == "SCAN": force_scan = True
...
app = Flask(__name__)
@app.route('/')
def home(): return "🤖 Agent V4.2 Online"
